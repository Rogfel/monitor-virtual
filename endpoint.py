...
def initialize_tool_agent():
    """Initialize the tool agent table with error recovery."""
    try:
        ...
        app.logger.warning(
            f"Could not connect to primary database table 'agents.tools': {str(e)}"
        )
        app.logger.info("Attempting to recover database...")
        
        try:
            # Try to recreate the database structure
            import subprocess
            import sys
            
            app.logger.info("Running database recovery...")
            result = subprocess.run([sys.executable, "setup_pixeltable.py"], 
                                  capture_output=True, text=True, timeout=300)
            
            if result.returncode == 0:
                app.logger.info("Database recovery successful, retrying connection...")
                tool_agent = pxt.get_table("agents.tools")
                if tool_agent is not None:
                    app.logger.info("Successfully connected to the tools (agent) table after recovery")
                    return tool_agent
                else:
                    raise RuntimeError("Database recovery failed - tools table still not accessible")
            else:
                app.logger.error(f"Database recovery failed: {result.stderr}")
                raise RuntimeError("Database recovery failed")
                
        except Exception as recovery_error:
            app.logger.error(
                f"FATAL: Could not recover database: {str(recovery_error)}",
                exc_info=True,
            )
            app.logger.error("Please run setup_pixeltable.py manually to fix the database")
            raise RuntimeError("Database is corrupted and cannot be recovered automatically")

# Initialize the tool agent
try:
    tool_agent = initialize_tool_agent()
except Exception as e:
    app.logger.error(
        f"FATAL: Could not initialize tool agent: {str(e)}",
        exc_info=True,
    )
...
def upload_file():
    ...
    try:
        ...
        app.logger.info(
            f"File '{filename}' (UUID: {file_uuid}) uploaded and added to '{config.TABLE_MAP[table_key]}' table for user {g.user_id}"
        )
        return jsonify(
            {
...
def add_url():
    ...
    try:
        ...
        app.logger.info(
            f"Added URL '{url}' (UUID: {file_uuid}) to '{config.TABLE_MAP[table_key]}' table for user {g.user_id}."
        )
        return jsonify(
            {
        ...
        try:
            ...
            if "timestamp" in wf_df.columns:
                ...
                try:
                    ...
                    wf_df["timestamp"] = pd.to_datetime(wf_df["timestamp"], errors='coerce')
                    # Include microseconds for accurate matching later
                    wf_df["timestamp"] = wf_df["timestamp"].dt.strftime(
                        "%Y-%m-%d %H:%M:%S.%f"
                    )
                except Exception as ts_err:
                    app.logger.warning(f"Error processing timestamp column: {str(ts_err)}")
                    # Fallback: convert to string if datetime conversion fails
                    wf_df["timestamp"] = wf_df["timestamp"].astype(str)
...
def delete_all():
    ...
    try:
        ...
        if not file_type or file_type not in config.TABLE_MAP:
            valid_types = ", ".join(config.TABLE_MAP.keys())
            return jsonify(
                {
        ...
        app.logger.info(
            f"Deleted {deleted_count} {file_type} items from {config.TABLE_MAP[file_type]} table for user {user_id}"
        )
        return jsonify(
            {
...
def delete_file_by_uuid(uuid, file_type):
    ...
    try:
        if file_type not in config.TABLE_MAP:
            valid_types = ", ".join(config.TABLE_MAP.keys())
